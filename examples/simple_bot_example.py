from boobs import Bot, TelegramAPIError, Token
from boobs.bot.rules import FromGroup, Fuzzy, Text, WasEdited
from boobs.bot.updates import ChatJoinRequest, Message
...
# or make your own, inheriting from boobs.bot.rules.abc.ABCRule.
...
async def message_handler(m: Message) -> None:
    ...
    # (see https://github.com/wondergram-org/boobs/blob/main/boobs/bot/updates/types.py#L43).
...
async def group_message_handler(m: Message) -> None:
    ...
    try:
        ...
        # (that can't be said for bot API, for example, while using nipples).
